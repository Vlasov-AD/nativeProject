...
<script>
      ...
      //если пустые поля
      const pop_submit_button = document.getElementById('submit_enter_form');

      pop_submit_button.addEventListener("click", (event)=>{
        if(document.getElementById('email_enter_form').value == 0 ||
            document.getElementById('password_enter_form').value == 0 ){
          alert('Вы не ввели данные!');
          event.preventDefault();
        }
      })

    </script> <!--Включение и выключение элементов -->